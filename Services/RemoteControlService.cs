...
public class RemoteControlService
{
    ...
    private const string RemoteChannel = "remote";

    // Server handler on shared direct TCP transport.
    public void StartServer()
    {
        if (_serverCts != null) return;
        _serverCts = new CancellationTokenSource();

        DirectTcpTransportService.Instance.RegisterHandler(RemoteChannel, HandleClientStreamAsync);
        _log.Info($"RemoteControlService server channel '{RemoteChannel}' active on TCP {DirectTcpTransportService.SharedPort}");
    }
    ...
    public void StopServer()
    {
        ...
        _serverCts.Dispose();
        _serverCts = null;
        DirectTcpTransportService.Instance.UnregisterHandler(RemoteChannel);
        _log.Info("RemoteControlService Server stopped");
    }

    private async Task HandleClientStreamAsync(Stream stream, CancellationToken ct)
    {
    ...
    public async Task<bool> ConnectToTargetAsync(Device targetDevice, string pkeyPath, CancellationToken ct)
    {
        ...
        try
        {
            _currentStream = await DirectTcpTransportService.Instance.ConnectAsync(targetDevice.IpAddress, RemoteChannel, ct);
        ...
        {
            _log.Error($"Failed to connect RemoteControl TCP stream: {ex.Message}");
            return false;
        }
    }
